...
<div class="row">
	<div class="pos-form-actions">
		<div class="col-md-12">
			...
			<!-- Botão para pagamento no PIX -->
			<button type="button" 
				class="btn bg-yellow btn-default btn-flat no-print @if(!empty($pos_settings['disable_suspend'])) @endif pos-express-finalize @if(!array_key_exists('bank_transfer', $payment_types)) hide @endif @if($is_mobile) col-xs-6 @endif" 
				data-pay_method="bank_transfer"
				title="@lang('Pagamento com PIX.')" >
				<i class="fas fa-address-card" aria-hidden="true"></i> @lang('PIX')
			</button>
			...
			@if(empty($edit))
				<button type="button" class="btn btn-danger btn-flat @if($is_mobile) col-xs-6 @else @endif" id="pos-cancel"> <i class="fas fa-window-close"></i> @lang('sale.cancel')</button>
			@else
				<button type="button" class="btn btn-danger btn-flat hide @if($is_mobile) col-xs-6 @else @endif" id="pos-delete"> <i class="fas fa-trash-alt"></i> @lang('messages.delete')</button>
			@endif
			...
			&nbsp;&nbsp;
			&nbsp;&nbsp;
			<span class="text-black text-right" style="font-size: 30px;" > <b>Total:&nbsp;</b></span>
			<input type="hidden" name="final_total" 
										id="final_total_input" value=0>
			<span id="total_payable" style="font-size: 40px; margin-top: 10px" class="text-white badge bg-red lead text-bold">0</span>			
			@endif
